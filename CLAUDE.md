...
- **Session 1 (core schema & canonical mounts): DONE.** Zod schemas, canonicalization + sha256 identity, 10 staple mount fixtures (incl. houdini and front mount; an 11th, dead-string-side, landed in Session 5), name registry with the brother/undermount alias, Cascade trick skeleton, 55 tests green. Identity includes finger digits and spin (front vs side) per owner direction; throws are graph entry points, not mount properties.
...
- **Session 5 (element library & authoring): DONE.** 12-element library with `difficulty` weights (1–5): mount, dismount, hop, hop-back (inverse pair), roll (topological self-edge — repeaters), pass ×{R index, L index, R thumb}, underpass ×2 (`under` wraps + `sweep: "under"` swing beneath the pivot), slack-pass ×2 (same endpoints as pass, different element — the multigraph's parallel edges; motion null, string-only). Element panel in the app = authoring view: all elements listed with difficulty dots, click-to-apply when legal, precondition reason when not; demos breakaway→DoN and trapeze rolls ×3. `dead-string-side` added as 11th fixture (the breakaway entry state). 121 tests green.
- **Session 5 post-review physics fixes:** swings follow a per-spin `SWING_DIRECTION` (momentum-consistent, never "shortest way"); rope collides only with fingers involved in either topology + the pivot (no palms — dragged string used to wrap the whole hand then snap to the finger); wraps pin apex-only (tangent-point pins could sit under the finger and pop the string over/under; collision shapes the flanks).
- **ARCHITECTURE PIVOT (read docs/PRIMITIVES.md before anything else): primitive movement animator.** Transitions-as-physics is retired; each edge becomes a specific primitive movement with a bespoke deterministic animation (`Anim = (t) => StringState`, pure and unit-testable), and mounts become the *outcomes* of movement sequences. Mount schemas/hashes/graph are TABLED — `src/core/` stays green but dormant until the primitives are proven, then re-attaches (primitive recipes emit mount schemas as postconditions). Scope: two working fingers only (index + middle per hand), side spin first.
- **Next up: primitives foundation + P1 (swing)** — `src/anim/` StringState with continuous `wrapAngle` attachments, geometry derivation on the fixed length budget, primitive playground panel in the app. Then one primitive per PR in docs/PRIMITIVES.md order. (Session 6 trick database is deferred until topology re-attaches.)
