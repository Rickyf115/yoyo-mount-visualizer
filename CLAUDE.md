...
- **Next up: Session 6 (trick database)** — SQLite via better-sqlite3: mounts keyed by canonical hash, mount_names, elements, tricks, trick_steps (nullable element_id); import fixtures; 5–10 real tricks incl. a same-endpoints/different-route pair; integrity test (pinned element maps step, or some legal element does for unpinned). Note: Cascade's trapeze→DoN step needs a single element or a step split — see IDEAS.
