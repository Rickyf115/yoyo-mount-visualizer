...
**Status:** Session 5 complete — core schema + canonical mounts, the 3D visualizer, string physics on one constant-length string, and a 12-element library with difficulty weights and an authoring panel.
...
Elements ([`src/core/elements.ts`](./src/core/elements.ts)) are the labeled edges of the mount multigraph: named operations `Mount → Mount` with legality **preconditions** (a machine-checkable reason-or-null). `applyElement` enforces the precondition and re-validates the result through the schema. Every element carries a **difficulty weight** (1–5) — not a pathfinding cost, but a filter/sort signal for browsing enumerated paths later. The library (12 elements):
...
- **`underpass under <finger>`** — the same insertion with an `under` wrap, scooping beneath the target: a genuinely different mount than the pass.
- **`slack pass`** — whip a loop of slack over the target instead of swinging the yo-yo. Topologically identical to the plain pass, so it is a **parallel edge** in the multigraph: same mount pair, different (harder) element — exactly the case the pathfinder must keep distinct.
- **`roll`** — a full loop around the supporting finger: the topological identity, i.e. a **self-edge**, which is what repeaters are made of.
- **`hop`** / **`hop-back`** — the yo-yo hops over the next/previous anchor onto the adjacent strand (mutual inverses).
- **`dismount`** — drop every wrap; back to a bare spinning string.

**Throws are entry points, not elements** (`THROWS`): a front throw enters the front-spin half of the graph, a breakaway the side-spin half, each as a spinning dead string (both halves' dead strings are fixtures). Elements never change spin — only future regeneration elements will cross the halves.
...
The **timeline bar** drives transitions: throw buttons enter the graph (front throw / breakaway) and a play/pause + scrubber controls the animation, with the traversal readout switching at the halfway beat. The **element panel** (left) is the authoring/debug view: every element in the library, with its difficulty and either a click-to-apply button or the precondition's reason it is illegal on the current mount — chain clicks to assemble a trick interactively and watch it play as one continuous burst. Element-produced mounts are recognized topologically (canonical serialization, no hashing needed in the browser), so mounting from a breakaway displays as "trapeze". The demo button plays the full trick from the throw: breakaway → mount → pass → pass into double or nothing.
